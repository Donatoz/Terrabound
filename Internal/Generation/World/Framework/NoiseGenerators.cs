...
namespace Metozis.TeTwo.Internal.Generation.World.Framework
{
    public static class NoiseGenerators
    {
	    ...
	    {
		    private readonly FastNoiseLite subGenerator;
		    private readonly float scale;

		    public FastNoiseGenerator(int seed, FastNoiseLite.NoiseType type, float scale = 1)
		    {
			    subGenerator = new FastNoiseLite();
			    this.scale = scale;
			    subGenerator.SetNoiseType(type);
			    subGenerator.SetSeed(seed);
		    }
		    ...
		    {
			    return subGenerator.GetNoise(x * scale, y * scale);
		    }
	    }
        ...
        {
	        private readonly Vector2 constraints;
	        private readonly float scale;
	        public PerlinNoiseGenerator(Vector2 constraints, float scale = 1)
	        {
            ...
            public float Generate(float x, float y)
            {
	            return Mathf.PerlinNoise(x / constraints.x * scale, y / constraints.y * scale);
            }
        }
        ...
        public class SimplexNoiseGenerator : INoiseGenerator
        {
			...
			public float coherentNoise(float x, float y, float z, int octaves=1, int multiplier = 25, float amplitude = 0.5f, float lacunarity = 2, float persistence = 0.9f) {
				...
				for (int n = 0; n < octaves; n++) {
				  val += Generate(v3.x,v3.y) * amplitude;
				  v3 *= lacunarity;
